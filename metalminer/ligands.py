            ...
            if HAS_RDKIT:
                try:
                    ...
                    possible_charges = [0, -1, -2, -3, -4, -5, -6, -8, -9, -10, 1, 2]
                                        # Chemistry-first selection (energy is only a tie-breaker)
                    candidates = []
                    fallback_mol = None
                    for q in possible_charges:
                        try:
                            ...
                            total_formal_charge = Chem.GetFormalCharge(candidate_mol)
                            radical_electrons = sum(a.GetNumRadicalElectrons() for a in candidate_mol.GetAtoms())
                            charged_atoms = sum(1 for a in candidate_mol.GetAtoms() if a.GetFormalCharge() != 0)
                            extreme_charge_atoms = sum(1 for a in candidate_mol.GetAtoms() if abs(a.GetFormalCharge()) > 1)

                            # ---- optional energy (tie-break only) ----
                            energy = None
                            ff_used = None
                                    ...
                                    if AllChem.MMFFHasAllMoleculeParams(candidate_mol):
                                        ...
                                        if mmff_props is not None:
                                            ...
                                            if mmff_ff is not None:
                                                mmff_ff.Minimize()
                                                energy = float(mmff_ff.CalcEnergy())
                                                ff_used = "MMFF94"
                                except Exception:
                                    pass
                                    ...
                                    try:
                                        if AllChem.UFFHasAllMoleculeParams(candidate_mol):
                                            ...
                                            if uff_ff is not None:
                                                uff_ff.Minimize()
                                                energy = float(uff_ff.CalcEnergy())
                                                ff_used = "UFF"
                                    except Exception:
                                        pass

                            # Rank chemistry first, then energy
                            rank_key = (
                                radical_electrons,                       # prefer fewer radicals
                                extreme_charge_atoms,                    # prefer fewer extreme ±2, ±3, ...
                                charged_atoms,                           # prefer less charge separation
                                abs(total_formal_charge),                # prefer smaller |charge| (unless you know expected charge)
                                0 if energy is not None else 1,          # prefer having an energy if available
                                ff_used != "MMFF94",                     # prefer MMFF over UFF as tie-break source
                                energy if energy is not None else float("inf")
                            )

                            candidates.append({
                                "q": q,
                                "mol": Chem.Mol(candidate_mol),
                                "rank_key": rank_key,
                                "formal_charge": total_formal_charge,
                                "radicals": radical_electrons,
                            ...
                            print(
                                f"q={q:>3} FC={total_formal_charge:>3} "
                                f"rad={radical_electrons} charged_atoms={charged_atoms} "
                                f"ff={ff_used} E={energy}"
                            )
                    ...
                    if candidates:
                        ...
                        rd_mol = best["mol"]
                        # optional debug
                       # print(f"[chosen] q={best['q']} FC={best['formal_charge']} ff={best['ff']} E={best['energy']}")
                    else:
                        rd_mol = fallback_mol
